...
class DishwasherManagerTest {
    ...
    void setUp() {
        ...
        dishwasherManager.addDishwasher(new GlasswareWasher("Glory-2 / F2", true, 80, 200, 2.54, true, 117));
    ...
    void getAllHigherThan() {
        var dishwashers = dishwasherManager.getAllHigherThan(100);
        assertEquals(1, dishwashers.size());

        for (var dishwasher : dishwashers) {
            assertTrue(dishwasher.getHeight() > 100);
        }
    }
}